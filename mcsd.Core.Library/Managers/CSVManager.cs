...
namespace mcsd.Library.Managers
{
    public class CSVManager
    {
        ...
        private string GetCSVExnacato()
        {
            ...
            this.DocumentHeader += string.Format("NombreCompleto{0}ProfesionOficio{0}Ciudad{0}", CSVManager.CSVSeparator);
            //
            using (StreamWriter sw = new StreamWriter(this.RutaDocumentoCSV, true, Encoding.Unicode))
            {
                ...
                for (int i = 0; i < rowCount; i++)
                {
                    ...
                    string ciudadPersona = maestroListado.Rows[i][3].ToString();
                    //
                    content.Append
                        (
                           string.Format("{0}{1}"
